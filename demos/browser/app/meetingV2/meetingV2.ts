...
export class DemoMeetingApp
    ...
    static readonly DATA_ACTION_MESSAGE_TOPIC: string = 'action';
    ...
    tileIndexToMuteEventListener: { [id: number]: (event: Event) => void } = {};
    ...
    constructor() {
        console.log('===start DemoMeetingApp Constructor');
        ...
        if (this.isRecorder() || this.isBroadcaster()) {
            AsyncScheduler.nextTick(async () => {
                ...
                this.meetingId = new URL(window.location.href).searchParams.get('meetingId');
                ...
                async (): Promise<void> => {
                    ...
                    try {
                        const response = await fetch(
                            `${DemoMeetingApp.BASE_URL}join?name=${encodeURIComponent(DemoMeetingApp.DID)}&region=${encodeURIComponent(
                                region
                            )}&meetingId=${encodeURIComponent(this.meetingId)}`,
                            {
                                method: 'POST',
                            }
                        );
                        ...
                        console.log('====sipURI ' + this.sipURI);
    ...
    setupSubscribeToAttendeeIdPresenceHandler(): void {
        ...
        this.activeSpeakerHandler = (attendeeIds: string[]): void => {
            console.log('=====now handle check active speakers!!!');
            if (attendeeIds.length > 0) {
                console.log('====most active speaker: ', attendeeIds[0]);
            }
            for (const attendeeId in this.roster) {
    ...
    dataMessageHandler(dataMessage: DataMessage): void {
        ...
        this.log('====start handle receive message!!!!', dataMessage);

        if (!dataMessage.throttled) {
            if (dataMessage.topic === "action"){
                this.log('===start handler receive action, action is ', dataMessage.text())
            } else {
                const isSelf =
    ...
    async joinMeeting(): Promise<any> {
        const response = await fetch(
            `${DemoMeetingApp.BASE_URL}join?title=${encodeURIComponent(
                this.meeting
            )}&name=${encodeURIComponent(this.name)}&region=${encodeURIComponent(this.region)}&meetingId=${encodeURIComponent(this.meetingId)}`,
            {
                method: 'POST',
            }
        );
    ...
    async endMeeting(): Promise<any> {
        await fetch(`${DemoMeetingApp.BASE_URL}end?meetingId=${encodeURIComponent(this.meetingId)}`, {
            method: 'POST',
        });
    }
    ...
    async authenticate(): Promise<string> {
        ...
        console.log('====join info and configuration ', joinInfo, configuration);
    ...
    createMuteUnMuteListener(tileState: VideoTileState): (event: Event) => void {
        return (event: Event): void => {
            console.log('====start send action mute other user', tileState);
            const sendMuteUnmuteMessage = (attendeeId: string): void => {
                AsyncScheduler.nextTick(() => {
                    this.audioVideo.realtimeSendDataMessage(
                        DemoMeetingApp.DATA_ACTION_MESSAGE_TOPIC,
                        attendeeId,
                        DemoMeetingApp.DATA_MESSAGE_LIFETIME_MS
                    );
                    // echo the message to the handler
                    this.dataMessageHandler(
                        new DataMessage(
                            Date.now(),
                            DemoMeetingApp.DATA_ACTION_MESSAGE_TOPIC,
                            new TextEncoder().encode(attendeeId),
                            this.meetingSession.configuration.credentials.attendeeId,
                            this.meetingSession.configuration.credentials.externalUserId
                        )
                    );
    ...
    videoTileDidUpdate(tileState: VideoTileState): void {
        ...
        const muteButtonElement = document.getElementById(
            `video-mute-${tileIndex}`
        ) as HTMLButtonElement;
        ...
        muteButtonElement.removeEventListener('click', this.tileIndexToMuteEventListener[tileIndex]);
        this.tileIndexToMuteEventListener[tileIndex] = this.createMuteUnMuteListener(tileState);
        muteButtonElement.addEventListener('click', this.tileIndexToMuteEventListener[tileIndex]);
